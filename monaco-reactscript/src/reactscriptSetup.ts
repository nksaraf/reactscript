...
let reactScriptWorker: (first: Uri, ...more: Uri[]) => Promise<ReactScriptWorker>;

export function setupReactScript(defaults: LanguageServiceDefaultsImpl): void {
	reactScriptWorker = setupMode(
		defaults,
		'reactscript'
	);
}

export function getReactScriptWorker(): Promise<(first: Uri, ...more: Uri[]) => Promise<ReactScriptWorker>> {
...
function setupMode(defaults: LanguageServiceDefaultsImpl, modeId: string): (first: Uri, ...more: Uri[]) => Promise<ReactScriptWorker> {
