...
const getFileMetaData = (dependency, version, depPath) => {
  if (UNPKG) {
    ...
    return doFetch(
      `https://unpkg.com/${dependency}@${version}${usedDepPath}?meta`
    )
...
const getFileTypes = (
  ...
  return doFetch(`${depUrl}${depPath}`).then(typings => {
    ...
    return Promise.all(
      dependencies
        .map(depPath =>
          fetchAndAddDependencies(
            getDependencyName(depPath),
            "latest",
            fetchedPaths
          ).catch(() => {
            /* ignore */
...
export async function fetchAndAddDependencies(dep, version, fetchedPaths = {}) {
  try {
