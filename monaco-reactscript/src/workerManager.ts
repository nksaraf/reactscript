...
export class WorkerManager {
	...
	private _worker: monaco.editor.MonacoWebWorker<ReactScriptWorker>;
	private _client: Promise<ReactScriptWorker>;
	...
	private _getClient(): Promise<ReactScriptWorker> {
		this._lastUsedTime = Date.now();

		if (!this._client) {
			this._worker = monaco.editor.createWebWorker<ReactScriptWorker>({

				// module that exports the create() method and returns a `TypeScriptWorker` instance
				moduleId: 'vs/language/typescript/reactscript',
			...
			let p = <Promise<ReactScriptWorker>>this._worker.getProxy();
	...
	getLanguageServiceWorker(...resources: Uri[]): Promise<ReactScriptWorker> {
		let _client: ReactScriptWorker;
