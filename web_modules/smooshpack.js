...
function dispatch(message) {
    if (!message)
        return;
    var newMessage = __assign$1(__assign$1({}, message), { codesandbox: true });
...
function notifyFrames(message) {
    ...
    bundlers.forEach(function (origin, frame) {
        if (frame && frame.postMessage) {
            frame.postMessage(__assign$1(__assign$1({}, rawMessage), { codesandbox: true }), origin);
        }
    });
}
...
var PreviewManager = /** @class */ (function () {
    function PreviewManager(selector, sandboxInfo, options) {
        ...
        this.iframe.setAttribute('sandbox', 'allow-forms allow-scripts allow-same-origin allow-modals allow-popups allow-presentation');
                ...
                case 'initialized': {
                    if (_this.iframe) {
                        if (_this.iframe.contentWindow) {
                            registerFrame(_this.iframe.contentWindow, _this.bundlerURL);
...
export { PreviewManager as Manager, createMissingPackageJSON as generatePackageJSON };
//# sourceMappingURL=smooshpack.js.map
